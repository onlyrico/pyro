...
def _Dependent__init__(self, *, is_discrete=NotImplemented, event_dim=NotImplemented):
    ...
    super(constraints._Dependent, self).__init__()
...
def _Dependent__call__(self, *, is_discrete=NotImplemented, event_dim=NotImplemented):
    ...
    return constraints._Dependent(is_discrete=is_discrete, event_dim=event_dim)
...
def _DependentProperty__init__(self, fn=None, *, is_discrete=NotImplemented, event_dim=NotImplemented):
    super(constraints._DependentProperty, self).__init__(fn)
    self._is_discrete = is_discrete
    self._event_dim = event_dim
...
def _DependentProperty__call__(self, fn):
    return constraints._DependentProperty(
...
def _AffineTransform__init__(self, loc, scale, event_dim=0, cache_size=0):
    super(torch.distributions.transforms.AffineTransform, self).__init__(
...
# backport of https://github.com/pytorch/pytorch/pull/50581
@patch_dependency('torch.distributions.transforms.AffineTransform.event_dim')
@_property
def _AffineTransform_event_dim(self):
    return self._event_dim


# backport of https://github.com/pytorch/pytorch/pull/50581
@patch_dependency('torch.distributions.transforms.AffineTransform.domain')
@constraints.dependent_property(is_discrete=False)
def _AffineTransform_domain(self):
    if self.event_dim == 0:
...
# backport of https://github.com/pytorch/pytorch/pull/50581
@patch_dependency('torch.distributions.transforms.AffineTransform.codomain')
@constraints.dependent_property(is_discrete=False)
def _AffineTransform_codomain(self):
    if self.event_dim == 0:
        return constraints.real
    return constraints.independent(constraints.real, self.event_dim)
